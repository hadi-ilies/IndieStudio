    ...
    for (uint i = 0; i < world.getSize().X; i++)
        for (uint j = 0; j < world.getSize().Y; j++)
            for (uint k = 0; k < world.getSize().Z; k++) {
                cerr << "pos : (" << i << ", " << j << ", " << k << ")" << endl;
                cerr << "block addr : " << world.getBlock(vector3du(i, j, k)) << endl;
                if (!world.getBlock(vector3du(i, j, k)))
                    continue;
                cerr << "block opaque : " << world.getBlock(vector3du(i, j, k))->getOpaque() << endl;
                if (world.getBlock(vector3du(i, j, k)) && !world.getBlock(vector3du(i, j, k))->getOpaque() && rand() % 2) {
                    ...
                    cerr << "new PowerUp : " << powerUpList.back()->getType() << endl;
                }
            }
