...
UserInterface::UserInterface() : irrFontBuffer(FONT_TYPE, GENERATE_FONT),
                                 ...
                                 prevMenu(nullptr), camera(nullptr), interfaceMusics(40),
                                 interfaceSounds(100), ipDisplayerMenu(nullptr), soundMenu(nullptr),
                                 musicMenu(nullptr), lock(false), myPlayer(
                new Player(nullptr, "Bomberman", USERNAME, nullptr, vector3du(0, 0, 0))),
                                 mapGame("Default") {
    //create sounds
 ...
 *   Methods
 */
/// @todo Make a factory to create the Menu
void UserInterface::linkButtonToMenu() {
    //create menus
    Menu *soloMenu = new Menu(window, irrFontBuffer, "Solo", vector3df(20, 53, 0), vector3df(10, 50, 0));
    Menu *multiPlayerMenu = new Menu(window, irrFontBuffer, "Multi", vector3df(70, 53, 0),
                                     vector3df(60, 50, 0));
    Menu *nbPlayerMenu = new Menu(window, irrFontBuffer, "nbPlayer", vector3df(130, 53, 0),
                                  vector3df(120, 50, 0));
    Menu *ipMenu = new Menu(window, irrFontBuffer, "ip", vector3df(-90, 53, 0), vector3df(-100, 50, 0));
    Menu *settingsMenu = new Menu(window, irrFontBuffer, "Settings", vector3df(20, -97, 0),
                                  vector3df(10, -100, 0));
    Menu *soundMenu = new Menu(window, irrFontBuffer, "Sound", vector3df(20, -397, 0),
                               vector3df(10, -400, 0));
    Menu *musicMenu = new Menu(window, irrFontBuffer, "Music", vector3df(20, -797, 0),
                               vector3df(10, -800, 0));
    Menu *modelMenu = new Menu(window, irrFontBuffer, "Player", vector3df(20, 503, 0),
                               vector3df(10, 500, 0));
    ...
    nbPlayerMenu->addWheel(vector3df(110, 50, 0), 10,
                           { "2", "3", "4", "5", "6", "7", "8", "9", "Back" }); //tmp
...
void UserInterface::checkVolume() {
    ...
    if (!musicLock) {
        delete musicMenu;
        musicMenu = new Menu(window, irrFontBuffer, "musicDisplayer", vector3df(20, -797, 0),
                             vector3df(10, -800, 0));
        musicMenu->addWheel(vector3df(0, -795, 0), 10,
                            { "", std::to_string((int) interfaceMusics) + "%" });
    }
    if (!soundLock) {
        delete soundMenu;
        soundMenu = new Menu(window, irrFontBuffer, "musicDisplayer", vector3df(20, -397, 0),
                             vector3df(10, -400, 0));
        soundMenu->addWheel(vector3df(0, -395, 0), 10,
                            { "", std::to_string((int) interfaceSounds) + "%" });
    }
}

void UserInterface::createTexture() {
    int i = 0;
    Menu *textureMenu = new Menu(window, irrFontBuffer, "texture", vector3df(20, 4003, 0),
                                 vector3df(10, 4000, 0));
...
void UserInterface::createMaps() {
    Menu *stageMenu = new Menu(window, irrFontBuffer, "stage", vector3df(20, 103, 0),
                               vector3df(10, 100, 0));
...
void UserInterface::checkPlayerFeatures() {
    if (menu->getKey() && menu->getName() == "Player") {
        delete myPlayer;
        myPlayer = new Player(&window, menu->getCurrentButtonModel(), USERNAME, nullptr,
                              vector3du(1, 1, 1));
        createTexture();
    } else if (menu->getKey() && menu->getName() == "texture")
        myPlayer->changeTexture(menu->getCurrentButtonTexture());
    else if (menu->getKey() && menu->getCurrentButtonName()
                               == "Stage") //Warning if create another button stage that do not do the same work its will be an issue
        createMaps();
...
void UserInterface::switchStatusMenu(const std::string &status) {
    Menu *statusMenu = new Menu(window, irrFontBuffer, status, vector3df(-480, -500, 0),
                                vector3df(-490, -500, 0));
...
void UserInterface::ipDisplayer(std::string &ipServer) {
    ipDisplayerMenu = new Menu(window, irrFontBuffer, "ipDisplayer", vector3df(-90, 53, 0),
                               vector3df(-100, 50, 0));
...
void UserInterface::selectIp(std::string &ipServer) {
    if (menu->getCurrentButtonName() == "SelectIp" && isIpV4Address(ipServer)) {
        ...
        changeScene(menu->getPosition(), menu->getMenu()->getPosition(),
                    menu->getMenu()->getTargetPosition());
        switchMenu();
        ...
        if (menu->getCurrentButtonName() == "."
            || (menu->getCurrentButtonName() != "Back" && menu->getCurrentButtonName() != "SelectIp"
                && menu->getCurrentButtonName() != "<==")) {
            delete ipDisplayerMenu;
...
void UserInterface::changeScene(const vector3df &cameraPos, const vector3df &cameraDestPos,
...
void UserInterface::playGame(const std::string &targetIp, const size_t &nbPlayer, const size_t &port,
                             const bool soloMode) {
    ...
    if (soloMode)
        for (size_t i = 1 ; i < nbPlayer ; i++) {
            ...
            myIaList.push_back(std::move(
                    std::make_unique<std::thread>(client, nullptr, aiPlayer, IACorentin,
                                                  sf::IpAddress(targetIp), port)));
...
void UserInterface::switchMenu() {
    ...
    if (menu->getName() == "texture" || menu->getName() == "stage" || menu->getName() == "Defeat"
        || menu->getName() == "Victory")
        delete menu;
    menu = tmpMenu;
}
