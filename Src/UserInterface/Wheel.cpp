 ...
 */
Wheel::Wheel(Window &_window, const vector3df &position, const float &radius,
             const std::vector <std::string> &buttonsNames, IrrFontBuffer &irrFontBuffer) : MenuElement(
        position, "Wheel"), radius(radius), currentButton(0), displayedButton(0) {
...
Wheel::Wheel(Window &_window, const vector3df &position, const float &radius,
             const std::vector <Wheel::ParamButton> &buttons) : MenuElement(position, "Wheel"),
                                                                radius(radius), currentButton(0) {
    for (uint i = 0 ; i < buttons.size() ; i++) {
        vector3df pos = getPosButton(i);
        buttonList.push_back(
                new Button(_window, pos, buttons[i].name, buttons[i].model, buttons[i].texture));
    }
    turnButtons(BASE, 1000);
}
...
void Wheel::turnButtons(const Wheel::Dir &direction, const f32 &timestamps) {
    if (buttonList[0]->isAnimationFinished()) {
        for (uint i = 0 ; i < buttonList.size() ; i++)
            buttonList[i]->animation(getPosButton(
                    (i + displayedButton + buttonList.size() + direction) % buttonList.size()),
                                     timestamps);
